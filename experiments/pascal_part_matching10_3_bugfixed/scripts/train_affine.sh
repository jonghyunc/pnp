...
./$SCRIPT_DIR/run.sh org.allenai.dqa.matching.TestMatchingCli --beamSize 100 --examples $TEST --diagrams $DIAGRAMS --diagramFeatures $DIAGRAM_FEATURES --model $MY_MODEL --lossJson $MY_DIR/test_error.json  > $MY_DIR/test_error_log.txt


echo "Testing $MY_NAME model on validation..."
VAL="$DATA_DIR/data_splits/$DATA_SPLIT/validation.json"
./$SCRIPT_DIR/run.sh org.allenai.dqa.matching.TestMatchingCli --beamSize 100 --examples $VAL --diagrams $DIAGRAMS --diagramFeatures $DIAGRAM_FEATURES --model $MY_MODEL --lossJson $MY_DIR/validation_error.json  > $MY_DIR/validation_error_log.txt
